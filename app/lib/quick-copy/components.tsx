...
const UNGROUPED_MENU_ITEM_ID = '$UNGROUPED'
  ...
  return (
    ...
    >
      <span className="block mb-4" {...editable.api.areaProps}>
        <input
          {...editable.api.inputProps}
          className={cx(
            '!text-base-content !rounded-btn',
      ...
      <span className="flex items-center space-x-4">
        ...
        >
          <span className={cx('swap', copied && 'swap-active')}>
            <span className="swap-on">
              <ClipboardCheckIcon className={cx(classes.icon, 'text-success')} />
            </span>
            ...
            <BaseForm
              ...
              onSubmit={e => {
                ...
                const success = moveString(
                  item.id,
                  groupSlug,
                  toGroupSlug === UNGROUPED_MENU_ITEM_ID ? null : toGroupSlug
                )
                if (success) moveDialog.api.close()
              }}
            >
              <FormControl>
                <FormLabel htmlFor={moveSelectId} {...moveDialog.api.descriptionProps}>
                  Which group do you want to move this string to?
                </FormLabel>
                <select
                  name="toGroup"
                  defaultValue={groupSlug ?? UNGROUPED_MENU_ITEM_ID}
                  className="select select-bordered w-full"
                >
                  <option value={UNGROUPED_MENU_ITEM_ID}>Remove from group</option>
                  {groups.map(g => (
        ...
        <Dialog api={deleteDialog.api}>
          <DialogBox>
            ...
            <DialogDescription>
              Are you sure you want to delete this string? This action cannot be undone.
            </DialogDescription>
            <DialogActions>
              <DialogCloseAction>Cancel</DialogCloseAction>
              <button className="btn btn-primary" onClick={() => deleteString(item.id, groupSlug)}>
                Delete
              </button>
            </DialogActions>
          </DialogBox>
...
export const CreateStringForm: React.FC<{ groupSlug: string | null; textareaId: string }> = ({
  groupSlug,
  textareaId,
}) => {
...
export const CopyBar: React.FC<{
  ...
  const [clearAfterCopy, setClearAfterCopy] = useLocalStorage('utils:copy-clear', false)
  const [copy, copied] = useCopy()
  return (
    <>
      <div>
      ...
      <div
        className={cx(
          'bottom-0 flex -m-4 p-4 rounded-box',
          selectedSet.size > 0 && 'sticky bg-base-100/50'
        )}
      >
        <button
        ...
        >
          <span className={cx('swap place-items-center', copied && 'swap-active')}>
            <span className="swap-off inline-flex space-x-2 items-center">
              <ClipboardCopyIcon className="w-6 h-6" />
              <span>
                Copy {selectedSet.size} {plural(selectedSet.size, 'string', 'strings')}
              </span>
            </span>
